...
- deploy ขึ้น GitHub Pages (ตอนนี้ deploy-from-branch: build → push `dist/` เข้า `gh-pages`)

### เปิด CI auto-deploy (ทางเลือก)

ไฟล์ workflow พร้อมใช้อยู่ที่ `ci-templates/github-pages-deploy.yml`
ย้ายไป `.github/workflows/deploy.yml` แล้วสลับ Pages source → **GitHub Actions**
ได้เมื่อ push ด้วย token ที่มี `workflow` scope (token ปัจจุบันไม่มี จึง deploy จาก branch ไปก่อน)
